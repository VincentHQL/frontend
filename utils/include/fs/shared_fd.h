#ifndef REDROID_LIBS_FS_SHARED_FD_H_
#define REDROID_LIBS_FS_SHARED_FD_H_
...
class FileInstance {
 ...
 public:
  ...
  ssize_t RecvFileDescriptors(void* buf, size_t len, size_t max_fds, std::vector<std::shared_ptr<FileInstance>>* recv_fds) {
    recv_fds->clear();
    std::vector<android::base::unique_fd> fds;
    errno = 0;
    auto ret = android::base::ReceiveFileDescriptorVector(fd_, buf, len, max_fds, &fds);
    std::vector<std::shared_ptr<FileInstance>> received_fds;
    for (size_t i = 0; i < fds.size(); i++) {
       received_fds.emplace_back(std::shared_ptr<FileInstance>(new FileInstance(fds[i].release(), errno)));
    }
    *recv_fds = std::move(received_fds);
...
#endif  // REDROID_LIBS_FS_SHARED_FD_H_
