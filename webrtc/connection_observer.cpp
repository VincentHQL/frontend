...
class ConnectionObserverImpl
    : public redroid::webrtc_streaming::ConnectionObserver {
 public:
  ConnectionObserverImpl(
      redroid::InputSockets &input_sockets,
      redroid::KernelLogEventsHandler *kernel_log_events_handler,
      std::map<std::string, redroid::SharedFD>
          commands_to_custom_action_servers,
      std::weak_ptr<DisplayHandler> display_handler,
      CameraController *camera_controller,
      redroid::confui::HostVirtualInput &confui_input)
  ...
  void OnTouchEvent(const std::string &display_label, int x, int y,
    ...
    redroid::WriteAll(input_sockets_.GetTouchClientByLabel(display_label),
  ...
  void OnKeyboardEvent(uint16_t code, bool down) override {
    ...
    buffer->AddEvent(EV_SYN, SYN_REPORT, 0);
    redroid::WriteAll(input_sockets_.keyboard_client,
                         reinterpret_cast<const char *>(buffer->data()),
                         buffer->size());
  }
  ...
  void OnSwitchEvent(uint16_t code, bool state) override {
    ...
    buffer->AddEvent(EV_SYN, SYN_REPORT, 0);
    redroid::WriteAll(input_sockets_.switches_client,
                         reinterpret_cast<const char *>(buffer->data()),
                         buffer->size());
  }
  ...
  void OnAdbChannelOpen(std::function<bool(const uint8_t *, size_t)>
    ...
    adb_handler_.reset(new redroid::webrtc_streaming::AdbHandler(
        redroid::RedroidConfig::Get()
            ->ForDefaultInstance()
            .adb_ip_and_port(),
        adb_message_sender));
  }
  ...
  void OnControlMessage(const uint8_t* msg, size_t size) override {
    ...
    if (command == "power") {
      ...
      redroid::WriteAll(commands_to_custom_action_servers_[command],
  ...
  void OnBluetoothChannelOpen(std::function<bool(const uint8_t *, size_t)>
    ...
    auto config = redroid::RedroidConfig::Get();
    CHECK(config) << "Failed to get config";
    bluetooth_handler_.reset(new redroid::webrtc_streaming::BluetoothHandler(
 ...
 private:
  redroid::InputSockets& input_sockets_;
  std::shared_ptr<redroid::webrtc_streaming::AdbHandler> adb_handler_;
  std::shared_ptr<redroid::webrtc_streaming::BluetoothHandler>
      bluetooth_handler_;
  std::map<std::string, redroid::SharedFD> commands_to_custom_action_servers_;
  std::weak_ptr<DisplayHandler> weak_display_handler_;
  std::set<int32_t> active_touch_slots_;
  redroid::CameraController *camera_controller_;
  redroid::confui::HostVirtualInput &confui_input_;
};
...
std::shared_ptr<redroid::webrtc_streaming::ConnectionObserver>
CfConnectionObserverFactory::CreateObserver() {
  return std::shared_ptr<redroid::webrtc_streaming::ConnectionObserver>(
      new ConnectionObserverImpl(input_sockets_, commands_to_custom_action_servers_,
                                 weak_display_handler_, camera_controller_,
                                 confui_input_));
}

void CfConnectionObserverFactory::AddCustomActionServer(
    redroid::SharedFD custom_action_server_fd,
