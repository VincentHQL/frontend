...
void Streamer::Impl::OnOpen() {
  ...
  signal_thread_->PostTask(RTC_FROM_HERE, [this]() {
    ...
    register_obj[redroid::webrtc_signaling::kTypeField] =
        redroid::webrtc_signaling::kRegisterType;
    register_obj[redroid::webrtc_signaling::kDeviceIdField] =
        config_.device_id;
    CHECK(config_.client_files_port >= 0) << "Invalide device port provided";
    register_obj[redroid::webrtc_signaling::kDevicePortField] =
    ...
    register_obj[redroid::webrtc_signaling::kDeviceInfoField] = device_info;
...
void Streamer::Impl::HandleClientMessage(const Json::Value& server_message) {
  CHECK(signal_thread_->IsCurrent())
      << __FUNCTION__ << " called from the wrong thread";
  if (!server_message.isMember(redroid::webrtc_signaling::kClientIdField) ||
      !server_message[redroid::webrtc_signaling::kClientIdField].isInt()) {
    LOG(ERROR) << "Client message received without valid client id";
    return;
  }
  auto client_id =
      server_message[redroid::webrtc_signaling::kClientIdField].asInt();
  if (!server_message.isMember(redroid::webrtc_signaling::kPayloadField)) {
    LOG(WARNING) << "Received empty client message";
    return;
  }
  auto client_message =
      server_message[redroid::webrtc_signaling::kPayloadField];
...
void Streamer::Impl::OnReceive(const uint8_t* msg, size_t length,
  ...
  signal_thread_->PostTask(RTC_FROM_HERE, [this, server_message]() {
    if (!server_message.isMember(redroid::webrtc_signaling::kTypeField) ||
        !server_message[redroid::webrtc_signaling::kTypeField].isString()) {
    ...
    auto type =
        server_message[redroid::webrtc_signaling::kTypeField].asString();
    if (type == redroid::webrtc_signaling::kConfigType) {
      HandleConfigMessage(server_message);
    } else if (type == redroid::webrtc_signaling::kClientDisconnectType) {
      if (!server_message.isMember(
              redroid::webrtc_signaling::kClientIdField) ||
    ...
    } else if (type == redroid::webrtc_signaling::kClientMessageType) {
...
void Streamer::Impl::SendMessageToClient(int client_id,
  ...
  CHECK(signal_thread_->IsCurrent())
      << __FUNCTION__ << " called from the wrong thread";
  Json::Value wrapper;
  wrapper[redroid::webrtc_signaling::kPayloadField] = msg;
  wrapper[redroid::webrtc_signaling::kTypeField] =
      redroid::webrtc_signaling::kForwardType;
  wrapper[redroid::webrtc_signaling::kClientIdField] = client_id;
